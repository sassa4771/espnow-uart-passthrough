...
**いちばん手早いのは、`uart_echo_demo.ino` をブロードキャストのまま、同じバイナリを 2 台の ESP32-C3 に書き込むことです。**  
`peerMac` のデフォルトはすでに `FF:FF:FF:FF:FF:FF`（ブロードキャスト）なので、MAC を調べて書き換える必要はありません。

### Arduino IDE でのボード設定（Espressif 公式パッケージ・XIAO ESP32-C3）

1. **ファイル → 環境設定**（英語 UI では **File → Preferences**、macOS は **Arduino IDE → 設定**）を開く
2. **追加のボードマネージャの URL**（Additional boards manager URLs）に次の 1 行を追加する（すでに他の URL がある場合は、行を分けて追加する）

   `https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json`

3. **ツール → ボード → ボードマネージャ** を開き、検索欄に `esp32` と入力して **esp32 by Espressif Systems** をインストールする（初回のみ）。**動作確認はこのパッケージのバージョン 3.3.7 のみです。** ボードマネージャでバージョンを選べる場合は 3.3.7 を指定してください。
4. **ツール → ボード** で **esp32** を展開し、**XIAO_ESP32C3** を選ぶ
5. USB 接続後、**ツール → ポート** で該当するシリアルポートを選ぶ（書き込み・シリアルモニタ用）

別メーカーの ESP32-C3 ボードを使う場合は、そのボード向けの定義を選んでください。

### スケッチの書き込みと疎通確認

1. Arduino IDE で `firmware/uart_echo_demo/uart_echo_demo.ino` を開く
2. 上記のとおり **XIAO_ESP32C3** とポートを選ぶ
3. 同じスケッチを **2 台とも** に書き込む
4. 両方のボードを USB で給電し、シリアルモニタを **115200 baud** で開く（2 つのウィンドウでよい）
5. 片方に 1 行入力して Enter を送ると、もう一方のシリアルに同じ行が出れば疎通できています

近距離・手早い確認ならこの構成で十分です。1 対 1 で相手を固定したい場合は、後述の `peerMac` を相手の実 MAC に変えてください。
...
- ボード種別: XIAO ESP32-C3
- Arduino ボードパッケージ: **esp32 by Espressif Systems** バージョン **3.3.7**（このバージョンでのみ動作確認済み。他バージョンは未確認です）
- 通信方式: ESP-NOW
- UART ボーレート: `115200`
